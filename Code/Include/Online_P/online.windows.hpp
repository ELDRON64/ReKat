...
( std::string name, std::string port ) {
    internal::this_name = (char*)calloc(32,sizeof(char));
...
( std::string new_name, std::string address ) {
    ...
    if ( internal::send_buf ( new_node, internal::this_name, 32, nullptr) == FAILED_SEND ) 
...
( ) {
	...
	if ( FD_ISSET( internal::ListenSocket, &FD_Listen ) ) {
        ...
        // read name
        char* _buf = (char*)calloc(32, sizeof(char));
        if (internal::recv_buf (new_node, _buf, 32, nullptr) == FAILED_RECV) 
