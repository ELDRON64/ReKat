...
static void grapik::Input::Keyboard ( GLFWwindow* window, int key, int scancode, int action, int mode ) {
    ...
    if ( ( action == GLFW_PRESS || action == GLFW_REPEAT ) ) {
        ...
        if ( key == GLFW_KEY_DELETE || key == GLFW_KEY_BACKSPACE ) { input.pop_back (); return; }
...
void RenderText(Shader &shader, std::string text, float init_x, float init_y, float scale, glm::vec3 color, float to_wrap, float wrap_h )
{
...
int main ( ) {
    ...
    while ( !Main_shutdown && grapik::IsEnd( ) ) {
        ...
        // command out;
        RenderText ( shader, "This is sample text", 25.0f, SCR_HEIGHT - 75.0f, 1.0f, glm::vec3(0.5, 0.8f, 0.2f), SCR_WIDTH - 50.0f, 50.0f );

        // command box
        RenderText ( shader, input, 25.0f, 100.0f, 0.5f, glm::vec3(0.3, 0.7f, 0.9f),SCR_WIDTH - 50.0f, 50.0f );
